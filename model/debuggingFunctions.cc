...
namespace ns3 {
    void printAllAgentsCosts(std::vector<Agent> &allAgents) {
        for (unsigned long int i = 0; i < globalInfo::allAgents.size(); i++) {
            std::cout << "Agent #" << i << ": ";
            allAgents.at(i).printAgentCosts();
        }
    }
    ...
    void prinAgentInfo(std::vector<Agent> &allAgents) {
        ...
        for (unsigned long int i = 0; i < globalInfo::allAgents.size(); i++) {
            std::cout << "Agent #" << allAgents.at(i).agentId << "\n" << "Class #" << allAgents.at(i).instrumentType
                    << "\n" << "Location: (" << allAgents.at(i).agentPosition.x << ", " << allAgents.at(i).agentPosition.y
                    << ", " << allAgents.at(i).agentPosition.z << ")\n";
        }
        std::cout << std::endl;
    }
    ...
    void printTaskInfo(std::vector<Task> &allTasks) {
        ...
        for (unsigned long int i = 0; i < globalInfo::allTasks.size(); i++) {
            std::cout << "Task #" << allTasks.at(i).taskId << "\n" << "Class #" << allTasks.at(i).instrumentRequirement << "\n" << "Location: (" << allTasks.at(i).taskLocation.x << ", " << allTasks.at(i).taskLocation.y << ", " << allTasks.at(i).taskLocation.z << ")\n";
        }
        std::cout << std::endl;
    }
