...
double Agent::distanceOfOptimal() {
    return euclideanDistance(originalPosition, agentPosition);
}
...
bool Agent::movingTowardSameTask(int whichAgent) {
    ...
    for (int i = 0; i < numTasks; i++) {
        ...
        double previousDistanceToTask = euclideanDistance(previousPosition, globalInfo::allTasks.at(i).task->taskLocation);
        Vector newPosition = knownPositions[whichAgent];
        double newDistanceToTask = euclideanDistance(newPosition, globalInfo::allTasks.at(i).task->taskLocation);
...
void Agent::fillInAgentCosts(std::vector<TaskNode> &allTasks) {
    ...
    for (int i = 0; i < numTasks; i++) {
        if (instrumentType == allTasks.at(i).task->instrumentRequirement) {
            taskCosts[i] = euclideanDistance(allTasks.at(i).task->taskLocation, agentPosition);
        }
...
bool Agent::agentIsOneHop(int whichAgent) {
    ...
    int howLongAgoHeard = currentMillis - lastTimeHeardFrom[whichAgent];
    double percentageToAsk = 100 - 100 * howLongAgoHeard / 5000;  // 5 seconds is max time to be considered one-hop
    int randNum = rand() % 100;
    if (randNum < percentageToAsk) {
        return true;
    }
    return false;
}
...
void Agent::resetLastTimeHeardFrom() {
    for (int i = 0; i < numAgents; i++) {
        lastTimeHeardFrom[i] = 0;
    }
}
...
void Agent::resetPositionsToAsk() {
    for (int i = 0; i < globalInfo::numAgents; i++) {
        positionsToAsk[i] = false;
    }
}
...
void Agent::resetKnownPositions() {
    for (int i = 0; i < numAgents; i++) {
        knownPositions[i] = Vector(0.0, 0.0, 0.0);
    }
    knownPositions[agentId] = agentPosition;
...
void Agent::resetPreviousKnownPositions() {
    for (int i = 0; i < numAgents; i++) {
        previousKnownPositions[i] = Vector(0.0, 0.0, 0.0);
    }
    previousKnownPositions[agentId] = agentPosition;
...
void Agent::resetPartialAssignment() {
    for (int i = 0; i < numAgents; i++) {
        partialAssignment[i] = -1;
    }
}
